...
/**
 * Event published when a user signs out.
 *
 ...
 * import type { UserSignOutEvent } from './UserSignOutEvent'
 *
 * const eventBus = inject<IEventBus>(EventBusKey)!
 *
 * // Subscribe to the event - on() returns an unsubscribe function
 * const unsubscribe = eventBus.on<UserSignOutEvent>('UserSignOut', (event) => {
 *   console.log('User signed out:', event.user.email.value)
 ...
 */
export class UserSignOutEvent implements DomainEvent {
  readonly eventId: Uint8Array
  readonly aggregateId: Uint8Array
  readonly eventName: string = 'UserSignOut'
